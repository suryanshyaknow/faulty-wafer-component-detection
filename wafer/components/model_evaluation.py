        ...
        try:
            ...
            if latest_dir is None:
                ...
                model_eval_artifact = artifact.ModelEvaluationArtifact(
                    are_models_replaced=True, improved_performances=None)
                lg.info(f"Model Evaluation Artifact: {model_eval_artifact}")
            else:
                lg.info('QUEST FOR KEEPING THE BETTER MODELS BETWEEN "JUST BUILT" MODELS AND "CURRENTLY DEPLOYED" ONES BEGINS NOW...')
                ...
                lg.info("loading the deployed version of artifacts from Model Registry...")
                ...
                lg.info("keeping only the features in the test set that were used in the training...")
                input_feats = list(latest_preprocessor.feature_names_in_)
                lg.info("..said features fetched succesfully!")
                # Preprocess Test Instances
                X_test_trans = latest_preprocessor.transform(X_test[input_feats])
                ...
                latest_scores = []
                for i in n_clusters:
                    lg.info(f'Gauging the true performance of model on "Cluster {i}" of test set...')

                    # Filter Cluster Instances
                    lg.info(f'filtering "Cluster {i}" instances..')
                    wafers_clus = wafers_clus[wafers_clus[:, -2] == i]
                    lg.info(f'Shape of "Cluster {i}" instances: {wafers_clus.shape}')

                    # Separate Features and Labels out
                    lg.info("Separating clustered features and labels out..")
                    X_test, y_test = np.delete(wafers_clus, [-2, -1], axis=1), wafers_clus[:, -1]
                    lg.info("..separated clustered features and labels successfully!")

                    # Load Cluster based Model
                    mod = BasicUtils.load_cluster_based_model(model_dir=latest_models_dir, cluster=i)
                    ...
                    if len(np.unique(y_test)) == 0:  # then can't use `roc_auc_score`, Will go ahead with `accuracy`
                        ...
                        lg.info(f'Accuracy on "Cluster {i}" of test set: {test_acc}')
                        latest_scores.append(test_acc)
                    else:  # gonna go ahead with the `roc_auc_score`
                        y_test_pred = mod.predict(X_test)
                        test_auc = roc_auc_score(y_test, y_test_pred)
                        lg.info(f'AUC score on "Cluster {i}" of test set: {test_auc}')
                        latest_scores.append(test_auc)
            
                lg.info(f"latest Models performances on test set: {latest_scores}")
